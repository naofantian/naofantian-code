...
INT GuardedMain( const TCHAR* CmdLine, HINSTANCE hInInstance, HINSTANCE hPrevInstance, INT nCmdShow )
{
	...
	const TCHAR *ConnectCmd = TEXT("+connect ");
	const INT ConnectCmdLength = appStrlen(ConnectCmd);
	if (appStrnicmp(CmdLine, ConnectCmd, ConnectCmdLength) == 0)
	{
		CmdLine += ConnectCmdLength;  // this makes the IP:port first on the command line, like Unreal expects.
	}
#endif
